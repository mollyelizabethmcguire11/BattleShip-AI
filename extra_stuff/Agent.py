...
class Agent:
    ...
    def update_qvalue(self,isHit,target,counter):
        ...
        sample = reward + self.discount * max(self.getQValue(action,isHit) for action in legalActions)
        self.qValues[(action,self.hits, self.shotsFired)] = (1-self.alpha) * self.getQValue(action,isHit) + self.alpha * sample
